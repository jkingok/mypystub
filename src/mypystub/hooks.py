...
class ScriptRunner:
    def __init__(
        self, app, input_label, input_field, print_field, toggle_input, toggle_print
    ):
        self.app = app
...
class Prototype:
    ...
    def get_content(self):
        ...
        input_txt = toga.TextInput(
            style=Pack(flex=1),
            on_gain_focus=lambda m: pad_keyboard(True),
            on_lose_focus=lambda m: pad_keyboard(False),
        )

        input_box = toga.Column(children=[prompt_lbl, input_txt], text_align="center")
        ...
        self.print_text = toga.MultilineTextInput(
            value="", readonly=True
        )  # , style=Pack(flex=1))
        ...
        self.script_runner = ScriptRunner(
            self.app, prompt_lbl, input_txt, self.print_text, toggle_input
        )
